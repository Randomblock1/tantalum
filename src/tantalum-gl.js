    ...
    init: function (gl, multiBufExt) {
        ...
        tgl.Shader = function (shaderDict, vert, frag) {
            ...
            if (!gl.getProgramParameter(this.program, gl.LINK_STATUS)) {
                var log = gl.getProgramInfoLog(this.program) || "(no program log)";
                throw new Error("Could not initialise shaders: " + log);
            }
        };
        ...
        tgl.Shader.prototype.createShaderObject = function (shaderDict, name, isFragment) {
            ...
            if (!gl.getShaderParameter(shaderObject, gl.COMPILE_STATUS)) {
                ...
                for (var i = 0; i < lines.length; ++i) lines[i] = ("   " + (i + 1)).slice(-4) + " | " + lines[i];
